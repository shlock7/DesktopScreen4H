...
uint16_t p, year_days;
uint8_t ping[13]={0,31,28,31,30,31,30,31,31,30,31,30,31};
void year_allday(uint16_t year)  //获取公历年初至某整月的天数
{
...
void update_day_count(uint16_t year, uint8_t month, uint8_t day){
    uint16_t start_year = 2023;
    uint8_t start_month = 2;
    uint8_t start_day = 25;
    uint16_t count_days = 0;
    
    year_allday(year);
    for(uint8_t m=0; m < month; m++)      //计算输入已过月份天数
        count_days += ping[m];
    count_days += day;

    year_allday(start_year);
    for(int n=0; n<start_month; n++)
        count_days -= ping[n];
    count_days -= start_day;
    for(int y=start_year; y<year; y++)
        {
            year_allday(y);
            year_days+=p;
        }
            count_days+=year_days;

    g_system_data.total_days = count_days;
}

void update_system_time(uint16_t year,uint8_t month,uint8_t day,uint8_t hour,uint8_t minute,uint8_t second){
    ESP_LOGI(TAG,"**********Today is %d-%d-%d **********\n",year,month,day);
    ...
    g_system_data.month = month;
    g_system_data.day = day;
    update_day_count(year, month, day);
    ESP_LOGI(TAG,"!!!!!!!! update love day %d !!!!!!!!\n",g_system_data.total_days);
}

void update_system_temperature(char* temperature){
    ESP_LOGI(TAG,"**********The temperature is %s **********",temperature);
    // char要先转换成uint
    g_system_data.temperature = check_chartime_to_uint(temperature, 2);
}
