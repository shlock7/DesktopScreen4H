...
static void cjson_time_info(char *text)
{
    ...
    update_system_time(year, month, day, hour,minute,second);
}
...
void cjson_weather_info(char *text)
{
    ...
    if(root!=NULL)
    {
        ...
        // 把温度传到system_data中，用于时钟页面更新
        update_system_temperature(weathe.temperatur);
    }
    cJSON_Delete(root);
}
...
void http_weather_get(){
    ...
    //判断是否有存储城市信息
    if(city != NULL){
        char url[] = "http://api.seniverse.com/v3/weather/now.json?key=SIjOhPf6iDQewmU1s&location=";
        ...
        strcpy(count_url,"http://api.seniverse.com/v3/weather/now.json?key=SIjOhPf6iDQewmU1s&location=qingdao&language=zh-Hans&unit=c");
    }
    //http client配置
    esp_http_client_config_t config = {
        .method = HTTP_METHOD_GET, //get请求
