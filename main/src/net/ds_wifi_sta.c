...
{
    s_wifi_event_group = xEventGroupCreate();  // 创建一个 FreeRTOS 事件组对象 

    // ESP_ERROR_CHECK(esp_netif_init());
    // ESP_ERROR_CHECK(esp_event_loop_create_default());

    esp_netif_create_default_wifi_sta();    // 创建默认的 WiFi Station网络接口
    // 使用默认配置初始化 WiFi 栈
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    ESP_ERROR_CHECK(esp_wifi_init(&cfg));
    // 注册 WiFi 事件和 IP 事件的事件处理程序
    ...
    wifi_config_t wifi_config = {
        .sta = {
	     ...
	     .threshold.authmode = WIFI_AUTH_WPA2_PSK, // 设置 STA 模式的认证模式为 WPA2-PSK
            .pmf_cfg = {
                .capable = true,
                .required = false
            },
    ...
    printf("\r\n");
    // 设置 WiFi 模式为 STA 模式，并将 WiFi 配置应用到 STA 接口，然后启动 WiFi
